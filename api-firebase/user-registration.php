...
if(isset($_POST['type']) && $_POST['type'] != '' && $_POST['type'] == 'edit-profile') {
    ...
    if (!empty($res)) {
		...
		$sql = 'UPDATE `users` SET `name`="'.$name.'",`email`="'.$email.'",`dob`="'.$dob.'",`city`="'.$city.'",`area`="'.$area.'",`latitude`="'.$latitude.'",`longitude`="'.$longitude.'",`mobile`="'.$mobile.'",`street`="'.$street.'",`pincode`="'.$pincode.'" WHERE `id`='.$id;
		$db->sql($sql);
