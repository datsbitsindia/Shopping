...
if(isset($_POST['get_all_user_data']) && $_POST['get_all_user_data'] != '') {
    
        $response = array();
        $sql_query = "SELECT *,(SELECT name FROM area a WHERE a.id=u.area) as area_name,(SELECT name FROM city c WHERE c.id=u.city) as city_name FROM `users` u";
        $db->sql($sql_query);
        $result=$db->getResult();
        if ($db->numRows($result) > 0) {
            ...
            foreach($result as $key => $row) {
                ...
                $response[$key]['user_id'] = $_SESSION['user_id']    = $row['id'];
                $response[$key]['name'] /*= $_SESSION['name']*/    = $row['name'];
                $response[$key]['email'] /*= $_SESSION['email']*/    = $row['email'];
                $response[$key]['mobile'] /*= $_SESSION['user']*/    = $row['mobile'];
                $response[$key]['dob'] = $row['dob'];
                $response[$key]['balance'] = $row['balance'];
                $response[$key]['city_id'] = !empty($row['city'])?$row['city']:'';
                $response[$key]['city_name'] = !empty($row['city_name'])?$row['city_name']:'';
                $response[$key]['area_id'] = !empty($row['area'])?$row['area']:'';
                $response[$key]['area_name'] = !empty($row['area_name'])?$row['area_name']:'';
                $response[$key]['street']     = $row['street'];
                $response[$key]['pincode']     = $row['pincode'];
                $response[$key]['referral_code']     = $row['referral_code'];
                $response[$key]['friends_code']     = $row['friends_code'];
                $response[$key]['apikey']     = $row['apikey'];
                $response[$key]['status']     = $row['status'];
                $response[$key]['created_at']     = $row['created_at'];
                // $_SESSION['timeout'] = $currentTime + $expired;
            }
        }else{
                $response['error']     = true;
