...
// 	echo $id;
	...
	if( empty($error['image']) ){
		...
		$sql = "INSERT INTO `slider_category1`(`image`,`type`, `type_id`) VALUES ('$upload_image','".$type."','".$id."')";
// 		echo $sql;
		// echo "a";
		// echo $sql;
		$db->sql($sql);
		$res = $db->getResult();
		$sql="SELECT id FROM `slider_category1` ORDER BY id DESC";
		$db->sql($sql);
		$res = $db->getResult();
...
if(isset($_GET['type']) && $_GET['type'] != '' && $_GET['type'] == 'delete-slider-category1') {
	...
	$sql = 'DELETE FROM `slider_category1` WHERE `id`='.$id;
	if($db->sql($sql)){
		echo 1;
	}else{
		echo 0;
...
if(isset($_POST['get-slider-images'])) {
	...
	$sql = 'select * from slider_category1 order by id desc';
